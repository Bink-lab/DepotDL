...
namespace DepotDL.GUI.Services
{
    ...
    public static class SteamMetadataService
    {
        ...
        private static readonly ConcurrentDictionary<string, string> _buildIdCache = new(StringComparer.OrdinalIgnoreCase);
        private static readonly ConcurrentDictionary<string, ConcurrentDictionary<string, string>> _manifestToBuildIdCache = new(StringComparer.OrdinalIgnoreCase);
        ...
        {
            if (manifestIds != null && _manifestToBuildIdCache.TryGetValue(appId, out var map))
            {
            ...
            if (_buildIdCache.TryGetValue(appId, out var bid)) return bid;
            return string.Empty;
        }
        ...
        private static Dictionary<string, SteamDepotMeta> TryLoad(string path, string appId)
        {
            ...
            try
            {
                ...
                if (depots.TryGetProperty("branches", out var branches) && branches.ValueKind == JsonValueKind.Object)
                {
                    ...
                    {
                        _buildIdCache[appId] = ReadBuildIdJson(publicBranch);
                    }
                    else
                    {
                        ...
                        {
                            _buildIdCache[appId] = ReadBuildIdJson(firstBranch.Value);
                        }
                    }
                }
                            ...
                            foreach (var app in r.Apps)
                            {
                                ...
                                if (depotsNode != KeyValue.Invalid)
                                {
                                    ...
                                    if (branchesNode != KeyValue.Invalid)
                                    {
                                        ...
                                        {
                                            _buildIdCache[appId] = publicBranch["buildid"].Value ?? string.Empty;
                                        }
                                        else
                                        {
                                            ...
                                            {
                                                _buildIdCache[appId] = firstBranch["buildid"].Value ?? string.Empty;
                                            }
                                        }
                                    }
        ...
        private static void ExtractManifestBuildIdsJson(string appId, JsonElement depots)
        {
            ...
            var map = _manifestToBuildIdCache.GetOrAdd(appId, _ => new(StringComparer.OrdinalIgnoreCase));
