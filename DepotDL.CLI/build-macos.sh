...
echo "Publishing DepotDL.CLI as self-contained single-file binary ($RID)..."
dotnet publish -c Release -r "$RID" --self-contained true /p:PublishSingleFile=true
find ../DepotDownloaderMod/bin/Release/net9.0/ -maxdepth 1 -type f ! -name "*.exe" -exec cp {} "bin/Release/net9.0/$RID/publish/" \;
